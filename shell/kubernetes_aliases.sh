...
alias kgel='k get events --sort-by=.lastTimestamp'
# get events sorted by creation timestamp
alias kgec='k get events --sort-by=.metadata.creationTimestamp'
...
alias kdebug="k -n default run debug-$USER --rm -it --tty --image leodotcloud/swiss-army-knife:v0.12 --image-pull-policy=IfNotPresent -- bash"
# get pod's containers list
function kgpc() { k get pod -o jsonpath="{.spec.containers[*].name}" "$@" && echo "" }
# ping a service, ex: 'kping whoami:8080'
alias kping='k run httping -it --image bretfisher/httping --image-pull-policy=IfNotPresent --rm=true --'
...
alias krmfailed='k delete pods --field-selector=status.phase=Failed'
